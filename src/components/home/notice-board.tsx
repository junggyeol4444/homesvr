  ...
  return (
    <section id="notices" className="glass-card space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-300">중요한 안내를 여기에서 확인할 수 있어요.</p>
      </div>
      <div className="space-y-4">
        {notices.length ? (
          notices.slice(0, 5).map((notice) => (
            <article key={notice.id} className="rounded-2xl border border-white/5 bg-black/30 p-4">
              <h3 className="text-base font-semibold text-white">{notice.title}</h3>
              <time className="mt-1 block text-xs text-slate-400">{formatDate(notice.createdAt)}</time>
              <p className="mt-2 text-sm text-slate-200 whitespace-pre-line">{notice.content}</p>
            </article>
          ))
        ) : (
