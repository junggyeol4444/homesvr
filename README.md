...
쌀가루집안 구성원의 소식과 일정을 관리하는 홈페이지입니다. 방송 일정, 멤버 소개, 공지, 하이라이트 VOD를 한 곳에서 관리·배포할 수 있도록 구성했습니다. 모든 콘텐츠는 정적 JSON/Markdown 데이터로 관리되어 Git 기반 워크플로우에 맞춰 버전 관리됩니다.

## 관리자 도구

- `/admin` 페이지에서 공지와 방송 일정을 직접 생성·삭제할 수 있습니다.
- 기본 관리자 비밀번호는 `change-me`이며, 운영 환경에서는 `ADMIN_PASSWORD` 환경 변수를 반드시 변경해주세요.
- 세션 암호화 키는 `ADMIN_SESSION_SECRET` 환경 변수로 설정할 수 있습니다.
